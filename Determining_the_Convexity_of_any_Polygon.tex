...
First we must assume that no edge doubles back on the previous edge. This means that \(\forall v \in P,\ T_P(v) < \pi\). For cases where this assumption does not hold, see section \ref{overlapping-edges}. This will allow us to assert that \(E\) is simple.
